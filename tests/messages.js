...
Test('/messages', function (t) {
    ...
    Parser.validate(apiPath, function (err, api) {
         ...
         */
        t.test('test getMessageByParams get operation', function (t) {
            Mockgen().requests({
                path: '/messages',
                operation: 'get'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/messages']['get']['responses']['200']['schema']);
         ...
         * responses: 200, default
         */
        t.test('test sendServiceMessage post operation', function (t) {
            Mockgen().requests({
                path: '/messages',
                operation: 'post'
            }, function (err, mock) {
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/messages']['post']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
